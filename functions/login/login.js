// Check for JWT token and redirect to login if not found
const jwt = sessionStorage.getItem("jwt");
if (!jwt && !window.location.href.includes("login.html")) {
...
const sendOTP = async () => {
    ...
    if (lastOTPSendTime && (currentTime - lastOTPSendTime) < 1800000) { // 30 minutes
        const remainingTime = Math.ceil((1800000 - (currentTime - lastOTPSendTime)) / 1000);
        Swal.fire({ title: 'กรุณารอสักครู่', text: `โปรดรอ ${remainingTime} วินาทีก่อนที่จะส่ง OTP อีกครั้ง`, icon: 'warning' });
        return;
    }

    const form = document.forms['login-form'];
    const enteredPassword = document.getElementById('password').value;
    const correctPassword = atob(atob(atob("WWxkV2EySllUakU9"))) + "18";

    if (enteredPassword !== correctPassword) {
        Swal.fire({ title: 'เข้าสู่ระบบล้มเหลว!', text: 'รหัสผ่านไม่ถูกต้อง', icon: 'error' });
        return;
    }
    ...
    try {
        ...
        sessionStorage.setItem("code", btoa(btoa(btoa(otpFromServer))));
        sessionStorage.setItem('lastOTPSendTime', currentTime);
    } catch (error) {
...
const login = () => {
    ...
    const correctUsername = atob(atob(atob("WWxkV2EySllUakU9")));
    const correctPassword = correctUsername + "18";
    const correctOTP = atob(atob(atob(code)));

    if (enteredUsername === correctUsername && enteredPassword === correctPassword && enteredOTP === correctOTP) {
        sessionStorage.setItem("jwt", "42392734892789fy8678326478t21tqwesgwhbhvc73y47ty1284g12u3g3uh12g3ugff9");
        Swal.fire({
            title: 'เข้าสู่ระบบสําเร็จ', text: 'กําลังเปลี่ยนเส้นทาง...', icon: 'success', showConfirmButton: false, timer: 1500,
            willClose: () => window.location.replace('Basic_phamaco.html')
        });
    } else {
        Swal.fire({ title: 'เข้าสู่ระบบล้มเหลว!', text: 'ชื่อผู้ใช้, รหัสผ่าน หรือ OTP ไม่ถูกต้อง', icon: 'error' });
    }
