...
function SendOTP() {
    ...
    // ตรวจสอบว่ามีเวลาที่ส่ง OTP ล่าสุดไว้ใน localStorage หรือไม่
    const lastOTPSendTime = localStorage.getItem('lastOTPSendTime');
    if (lastOTPSendTime) {
        const currentTime = new Date().getTime();
        const timeDiffInSeconds = (currentTime - lastOTPSendTime) / 1000;

        // ตรวจสอบว่าผ่านไปเวลามากกว่า 30 นาทีหรือไม่
        if (timeDiffInSeconds < 60) { // 60 คือ 30 นาทีในหน่วยวินาที
            const remainingTime = Math.ceil(60 - timeDiffInSeconds);
            Swal.fire({
    ...
    const scriptURL = 'https://script.google.com/macros/s/AKfycby0TJmsjunj-tMLZ5jzSYVQCIapTnGRqewiqWA3_Qqzw2qGApCkJc_gdeeKyDsFez2p/exec';
            ...
            if (timeDiffInSeconds >= 60) {
                fetch(scriptURL, { method: 'POST', body: new FormData(form) })
                    ...
                    .then(data => {
                        ...
                        // เก็บเวลาที่ส่ง OTP ล่าสุดไว้ใน localStorage
                        localStorage.setItem('lastOTPSendTime', new Date().getTime());
                    })
